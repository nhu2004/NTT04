...
// Lấy danh sách các mục lọc
var sizeFilters = document.querySelectorAll('.filter_size .Linklist__Item');
var priceFilters = document.querySelectorAll('.filter_price .Linklist__Item');

// Hàm để xử lý sự kiện click
function handleClick(event) {
    // Lấy giá trị của mục lọc được chọn
    var size = document.querySelector('.filter_size .Linklist__Item.active').innerText;
    var price = document.querySelector('.filter_price .Linklist__Item.active').innerText;

    // Tạo URL mới
    var newUrl = 'spdensize41.html';

    // Chuyển hướng đến URL mới
    window.location.href = newUrl;
}

// Thêm sự kiện click cho các mục lọc
...
let increaseButtons = document.querySelectorAll('.minicart-step-up');
let decreaseButtons = document.querySelectorAll('.minicart-step-down');
let quantityInputs = document.querySelectorAll('.pro-quantity-view');

// Thêm sự kiện click cho các nút tăng số lượng
increaseButtons.forEach((button, index) => {
    button.addEventListener('click', function() {
        if (quantityInputs[index].value < 5) {
